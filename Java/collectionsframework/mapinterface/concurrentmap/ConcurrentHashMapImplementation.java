...
public class ConcurrentHashMapImplementation {
    ...
    // now Thread A wants to change it's value to 60
    ...
    // if it is still 50 it means no other thread is woking on it, so it wil change the value to 60
    ...
    // so next time it will again compare the value of x with it's last seen value, which was now 60
    // and if it is still same, it is safe to change it's value
